...
@section('content')
<div x-data="reminderForm(true)" class="mx-auto max-w-6xl space-y-6">
    ...
    <div>
        <nav class="mb-2 flex items-center gap-1.5 text-xs font-medium text-slate-400" aria-label="Breadcrumb">
            ...
            <span class="font-semibold text-slate-600">Edit Reminder</span>
        </nav>
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Edit Reminder</h2>
        <p class="mt-0.5 text-sm text-slate-500">Perbarui sasaran, jadwal poli, home visit, dan pesan pengingat.</p>
    </div>
        ...
        <div class="space-y-6 lg:col-span-2">
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="border-b border-slate-100 p-6">
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">Cari dan pilih satuan kerja sasaran broadcast.</p>
                </div>
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="border-b border-slate-100 p-6">
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">Ambil dari database PNPP satker terpilih, atau gunakan select grouping.</p>
                </div>
                ...
                <div class="space-y-5 p-6">
                    ...
                    <div x-show="mode === 'pnpp' && satkerId !== null" x-cloak>
                        ...
                        <p class="mt-2 text-xs text-slate-400"><span x-text="pilih.length"></span> pasien dipilih — 1 pasien bisa dijadwalkan ke beberapa poli.</p>
                    </div>
            ...
            {{-- ===== Langkah 3: Jadwal Kunjungan & Poli ===== --}}
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="border-b border-slate-100 p-6">
                    <h3 class="flex items-center gap-2.5 text-base font-bold text-slate-900">
                        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-sky-600 text-xs font-bold text-white">3</span>
                        Jadwal Kunjungan & Poli
                    </h3>
                    <p class="mt-0.5 text-sm text-slate-500">Satu pasien bisa dijadwalkan ke beberapa poli. Home visit bersifat opsional.</p>
                </div>

                <div class="space-y-4 p-6">
                   

                    <div x-show="pilih.length === 0" x-cloak class="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-8 text-center">
                        <p class="text-sm text-slate-400">Belum ada pasien dipilih.</p>
                    </div>

                    <template x-for="pasien in pilih" :key="pasien.id">
                        <div class="rounded-xl ring-1 ring-slate-200">
                            ...
                            <div class="flex flex-wrap items-center gap-3 border-b border-slate-100 bg-slate-50/60 px-4 py-3">
                                <span class="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold uppercase text-emerald-700" x-text="inisial(pasien.nama)"></span>
                                <div class="min-w-0 flex-1">
                                    <p class="truncate text-sm font-bold text-slate-900" x-text="pasien.nama"></p>
                                    <p class="truncate font-mono text-xs text-slate-400" x-text="pasien.nip || pasien.noHp || '—'"></p>
                                </div>
                                <button type="button" @click="togglePnpp(pasien)" title="Hapus dari daftar" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                                </button>
                            </div>

                            <div class="space-y-4 p-4">
                                {{-- Poli (bisa pilih beberapa) --}}
                                <div>
                                    <p class="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Poli Tujuan <span class="text-slate-400 normal-case font-medium">(bisa lebih dari satu)</span></p>
                                    <div class="flex flex-wrap gap-2">
                                        <template x-for="poli in polis" :key="poli.id">
                                            <button type="button" @click="togglePoli(pasien, poli.id)"
                                                    class="rounded-full px-3 py-1.5 text-xs font-semibold ring-1 ring-inset transition"
                                                    :class="pasien.poli.includes(poli.id) ? 'bg-sky-600 text-white ring-sky-600' : 'bg-white text-slate-600 ring-slate-200 hover:bg-slate-50'"
                                                    x-text="poli.nama"></button>
                                        </template>
                                        <p x-show="polis.length === 0" class="text-xs text-slate-400">Belum ada data poli.</p>
                                    </div>
                                </div>

                                <div>
                                    <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Tanggal Kunjungan</label>
                                    <input type="date" x-model="pasien.tglKunjungan"
                                 ...
                                 {{-- Home visit (opsional, radio) --}}
                    <div class="rounded-xl bg-slate-50/70 p-4 ring-1 ring-slate-100">
                        <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Home Visit <span class="font-medium normal-case tracking-normal text-slate-400">(opsional)</span></p>
                        <div class="mt-3 grid grid-cols-2 gap-2">
                            <label class="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white px-3.5 py-2.5 ring-1 ring-inset transition"
                                   :class="homeVisit === 'tidak' ? 'ring-2 ring-sky-500' : 'ring-slate-200 hover:bg-slate-50'">
                                <input type="radio" name="homeVisit" value="tidak" x-model="homeVisit" class="h-4 w-4 border-slate-300 text-sky-600 focus:ring-sky-500">
                                <span class="text-sm font-semibold text-slate-700">Tidak</span>
                            </label>
                            <label class="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white px-3.5 py-2.5 ring-1 ring-inset transition"
                                   :class="homeVisit === 'ya' ? 'ring-2 ring-emerald-500' : 'ring-slate-200 hover:bg-slate-50'">
                                <input type="radio" name="homeVisit" value="ya" x-model="homeVisit" class="h-4 w-4 border-slate-300 text-emerald-600 focus:ring-emerald-500">
                                <span class="text-sm font-semibold text-slate-700">Ya</span>
                                <span class="ml-auto hidden text-[10px] font-semibold text-slate-400 sm:inline">ada jadwal ke rumah</span>
                            </label>
                        </div>
                    </div>
                            </div>
            ...
            {{-- <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="border-b border-slate-100 p-6">
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">Atur isi pesan WhatsApp dan waktu pengiriman untuk penjadwalan di atas.</p>
                </div>
                ...
                <div class="space-y-5 p-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-xs text-slate-400">Variabel tersedia: <code class="rounded bg-slate-100 px-1 py-0.5 font-mono">{nama}</code> <code class="rounded bg-slate-100 px-1 py-0.5 font-mono">{poli}</code> <code class="rounded bg-slate-100 px-1 py-0.5 font-mono">{tanggal}</code></p>
                    </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.digital-reminder.index') }}" class="rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-xs transition hover:bg-slate-50">Batal</a>
        ...
        <div class="space-y-6 lg:col-span-1">
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                ...
                <ul class="divide-y divide-slate-50">
                    ...
                    <li class="flex items-center justify-between px-5 py-3.5">
                        <span class="text-sm font-medium text-slate-500">Pasien dipilih</span>
                        <span class="text-sm font-bold tabular-nums text-slate-800" x-text="pilih.length"></span>
                    </li>
                    <li class="flex items-center justify-between px-5 py-3.5">
                        <span class="text-sm font-medium text-slate-500">Total jadwal poli</span>
                        <span class="text-sm font-bold tabular-nums text-slate-800" x-text="totalPoli"></span>
                    </li>
                    <li class="flex items-center justify-between px-5 py-3.5">
                        <span class="text-sm font-medium text-slate-500">Home visit</span>
                        <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset"
                              :class="homeVisit === 'ya' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200/70' : 'bg-slate-50 text-slate-400 ring-slate-200'"
                              x-text="homeVisit === 'ya' ? 'Ya' : (homeVisit === 'tidak' ? 'Tidak' : '—')"></span>
                    </li>
                </ul>
            </div>
            ...
            {{-- <div class="sticky top-20 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                ...
                <div class="space-y-3 bg-[#e5ddd5] p-5">
                    ...
                    <p x-show="pilih.length === 0" x-cloak class="py-4 text-center text-xs text-slate-500">Pilih pasien untuk melihat pratinjau pesan.</p>
                </div>
            </div> --}}
        </div>
    </div>
...
<script>
    function reminderForm(prefill = false) {
        return {
            ...
            pilih: [],
            templateId: null,
            pesan: 'Halo {nama}, Anda memiliki jadwal kunjungan di {poli} pada {tanggal}. Mohon hadir tepat waktu.',
            jadwalKirim: '',
            modeKirim: 'terjadwal',
            status: 'Terkirim',
            ...
            init() {
                if (!prefill) return;
                // Prefill demo: satker pertama + 2 PNPP pertamanya (disesuaikan saat logic tersedia).
                this.satkerId = this.satkers[0]?.id ?? null;
                const sasaran = this.pnpps.filter(p => p.satkerId === this.satkerId).slice(0, 2);
                this.pilih = sasaran.map((p, i) => ({
                    id: p.id, nama: p.nama, nip: p.nip, noHp: p.noHp,
                    poli: this.polis.slice(0, i === 0 ? 2 : 1).map(x => x.id),
                    tglKunjungan: this.hariIni(),
                }));
                this.homeVisit = 'ya';
                this.templateId = this.templates[0]?.id ?? null;
                if (this.templates[0]) this.pesan = this.templates[0].konten;
                this.jadwalKirim = this.hariIni() + 'T07:30';
            },
            hariIni() {
            ...
            get totalPoli() {
                return this.pilih.reduce((n, p) => n + p.poli.length, 0);
            },
            get previewPesan() {
                const p = this.pilih[0];
                if (!p) return this.pesan;
                let teks = this.pesan;
                teks = teks.replaceAll('{nama}', p.nama.split(' ')[0]);
                teks = teks.replaceAll('{poli}', p.poli.length ? p.poli.map(id => this.namaPoli(id)).join(', ') : '{poli}');
                teks = teks.replaceAll('{tanggal}', p.tglKunjungan ? this.formatTanggal(p.tglKunjungan) : '{tanggal}');
                if (this.homeVisit === 'ya') {
                    teks += '\n\nKami juga menjadwalkan home visit untuk kunjungan ini.';
                }
                return teks;
            },
            get previewWaktu() {
                if (!this.jadwalKirim) return '';
                const d = new Date(this.jadwalKirim);
            ...
            namaPoli(id) {
                return this.polis.find(p => p.id === id)?.nama || '—';
            },
            terpilih(id) {
            ...
            togglePnpp(p) {
                ...
                if (i >= 0) {
                    ...
                    this.pilih.push({ id: p.id, nama: p.nama, nip: p.nip, noHp: p.noHp, poli: [], tglKunjungan: '' });
                }
            },
            pilihSemua() {
            ...
            applyGrouping() {
                ...
                this.pilih = this.pnpps
                    ...
                    .map(p => ({ id: p.id, nama: p.nama, nip: p.nip, noHp: p.noHp, poli: [], tglKunjungan: '' }));
            },
            togglePoli(pasien, poliId) {
                const i = pasien.poli.indexOf(poliId);
                i >= 0 ? pasien.poli.splice(i, 1) : pasien.poli.push(poliId);
            },
            applyTemplate() {
            ...
            formatTanggal(iso) {
                return new Date(iso + 'T00:00:00').toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' });
            },
        };
    }
</script>
