...
@php
    $stats = [
        ['label' => 'Total Jadwal', 'value' => number_format($total),                                   'tone' => 'sky',     'icon' => 'M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0'],
    ...
    $statusLabel = ['terjadwal' => 'Terjadwal', 'selesai' => 'Selesai', 'tidak_datang' => 'Tidak Datang', 'dibatalkan' => 'Dibatalkan'];
@endphp
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-0.5 text-sm text-slate-500">Penjadwalan kunjungan pasien — pesan outreach &amp; follow up digenerate dari jadwal ini.</p>
        </div>
    ...
    {{-- ===== Daftar jadwal + filter ===== --}}
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4">
            <h3 class="text-sm font-bold text-slate-900">Daftar Penjadwalan</h3>
        </div>

        <form method="GET" action="{{ route('admin.digital-reminder.index') }}"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.digital-reminder.index') }}" class="rounded-lg bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-300">Reset</a>
            </div>
        </form>

        @if ($reminders->isEmpty())
            <div class="px-5 py-12 text-center">
                <p class="text-sm font-medium text-slate-500">Belum ada penjadwalan kunjungan.</p>
                <a href="{{ route('admin.digital-reminder.create') }}" class="mt-3 inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700">
                    Buat jadwal pertama
                </a>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($reminders as $r)
                            <tr class="hover:bg-slate-50/60">
                                ...
                                <td class="px-5 py-3">
                                    <p class="text-xs font-medium text-slate-600">{{ $r->poli?->nama ?? '—' }}</p>
                                    <p class="text-xs text-slate-400">{{ $r->dokter?->nama ?? 'Dokter belum ditentukan' }}</p>
                                </td>
                                <td class="px-5 py-3">
                                    <span class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 ring-inset {{ $r->home_visit ? 'bg-teal-50 text-teal-700 ring-teal-200/70' : 'bg-slate-100 text-slate-500 ring-slate-200/70' }}">
                                        {{ $r->home_visit ? 'Home Visit' : 'Kunjungan RS' }}
                                    </span>
                                </td>
                                <td class="px-5 py-3">
                                ...
                                <td class="whitespace-nowrap px-5 py-3 text-right">
                                    ...
                                    @endif
                                    <a href="{{ route('admin.digital-reminder.edit', $r) }}"
                                       class="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-200">
                                        Edit
                                    </a>
                                    <button type="button"
                                            onclick="confirmSubmit('{{ route('admin.digital-reminder.destroy', $r) }}', {
                                                title: 'Hapus penjadwalan?',
                                                html: 'Jadwal <strong>{{ $r->pnpp?->nama }}</strong> pada {{ $r->tanggal?->translatedFormat('d M Y') }} akan dihapus permanen.',
