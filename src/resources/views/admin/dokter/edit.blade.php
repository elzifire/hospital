...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        <nav class="mb-2 flex items-center gap-1.5 text-xs font-medium text-slate-400" aria-label="Breadcrumb">
            ...
            <span class="font-semibold text-slate-600">{{ $dokter->nama }}</span>
        </nav>
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Edit Dokter</h2>
        <p class="mt-0.5 text-sm text-slate-500">Perbarui informasi dokter dan polinya.</p>
    </div>

    <form action="{{ route('admin.dokter.update', $dokter->id) }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
        @method('PUT')
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div class="space-y-5 p-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $dokter->nama) }}" required placeholder="cth. dr. Rina Pratiwi"
                ...
                <div>
                    ...
                    <input type="text" name="spesialisasi" id="spesialisasi" value="{{ old('spesialisasi', $dokter->spesialisasi) }}" placeholder="cth. Spesialis Penyakit Dalam"
                ...
                <div>
                    ...
                    <select name="poli_id" id="poli_id"
                        ...
                        @foreach ($polis as $poli)
                            <option value="{{ $poli->id }}" @selected(old('poli_id', $dokter->poli_id) == $poli->id)>{{ $poli->nama }}</option>
                        @endforeach
                    </select>
