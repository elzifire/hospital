...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-0.5 text-sm text-slate-500">Pesan undangan jadwal (H-7 &amp; H-1) yang digenerate dari penjadwalan digital reminder.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            @can('manage outreach')
                <form method="POST" action="{{ route('admin.outreach.generate') }}">
                    @csrf
                    <button type="submit"
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        @if ($logs->isEmpty())
            <div class="px-5 py-12 text-center">
                <p class="text-sm font-medium text-slate-500">Belum ada pesan outreach. Buat jadwal di Digital Reminder lalu generate.</p>
                @can('manage outreach')
                    <form method="POST" action="{{ route('admin.outreach.generate') }}" class="mt-3">
                        @csrf
