...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        ...
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Tambah Jadwal Dokter</h2>
        <p class="mt-0.5 text-sm text-slate-500">Atur hari dan jam praktik dokter di polinya.</p>
    </div>

    <form action="{{ route('admin.jadwal.store') }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                ...
                <div>
                    ...
                    @if ($dokters->isEmpty())
                        <p class="rounded-xl bg-slate-50 p-4 text-sm text-slate-500 ring-1 ring-slate-100">
                            Belum ada data dokter. <a href="{{ route('admin.dokter.create') }}" class="font-semibold text-sky-600 hover:underline">Tambahkan dulu</a>.
                        </p>
                    @else
                        <select name="dokter_id" id="dokter_id"
                            ...
                            @foreach ($dokters as $dokter)
                                <option value="{{ $dokter->id }}" @selected(old('dokter_id') == $dokter->id)>{{ $dokter->nama }} — {{ $dokter->poli?->nama }}</option>
                            @endforeach
                        </select>
            ...
            <div class="flex items-center justify-end gap-3 rounded-b-2xl border-t border-slate-100 bg-slate-50/70 p-4">
                ...
                <button type="submit" :disabled="saving"
                    ...
                    <span x-text="saving ? 'Menyimpan...' : 'Simpan Jadwal'"></span>
                </button>
            </div>
        </div>
    </form>
