...
@section('page-title', 'Edit Satker')
...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        <nav class="mb-2 flex items-center gap-1.5 text-xs font-medium text-slate-400" aria-label="Breadcrumb">
            <a href="{{ route('admin.satker.index') }}" class="rounded transition hover:text-sky-600">Data Satker</a>
            ...
            <span class="font-semibold text-slate-600">{{ $satker->nama }}</span>
        </nav>
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Edit Satker</h2>
        <p class="mt-0.5 text-sm text-slate-500">Perbarui informasi satuan kerja.</p>
    </div>

    <form action="{{ route('admin.satker.update', $satker->id) }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
        @method('PUT')
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-100 p-6">
                <h3 class="text-base font-bold text-slate-900">Informasi Satker</h3>
                <p class="mt-0.5 text-sm text-slate-500">Kode bersifat opsional, nama wajib diisi.</p>
            </div>

            <div class="space-y-5 p-6">
                <div>
                    <label for="kode" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Kode Satker</label>
                    <input type="text" name="kode" id="kode" value="{{ old('kode', $satker->kode) }}" placeholder="cth. DINKES"
                ...
                <div>
                    <label for="nama" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Nama Satker <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $satker->nama) }}" required placeholder="cth. Dinas Kesehatan"
