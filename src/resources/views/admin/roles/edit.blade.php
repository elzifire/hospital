...
<div x-data="roleForm()" class="mx-auto max-w-7xl space-y-6">
    ...
    <div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <h2 class="text-xl font-bold tracking-tight text-slate-900">Edit Role</h2>
            <span class="inline-flex items-center gap-1.5 rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-bold text-sky-700 ring-1 ring-inset ring-sky-600/20">
                ...
                {{ $role->users_count }} user memakai role ini
            </span>
        </div>
        <p class="mt-0.5 text-sm text-slate-500">Perubahan berlaku untuk semua user yang memakai role ini.</p>
    </div>

    <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" @submit="saving = true">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6 lg:col-span-2">
                <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                    <div class="border-b border-slate-100 p-6">
                        ...
                        <p class="mt-0.5 text-sm text-slate-500">Ubah nama role bila diperlukan.</p>
                    </div>
                    <div class="p-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" x-model="name" required placeholder="cth. Dokter, Kasir, Perawat"
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-20 space-y-4">
                    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                        ...
                        <div class="flex items-center gap-3.5 px-5 py-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-slate-400"><span class="font-bold tabular-nums text-sky-600" x-text="selected.length">{{ $role->permissions->count() }}</span> dari {{ $permissions->count() }} permission</p>
                            </div>
                        </div>
                        <div class="px-5 pb-4">
                    ...
                    {{-- Warning jika role dipakai --}}
                    @if ($role->users_count > 0)
                        <div class="flex items-start gap-3 rounded-xl bg-amber-50 p-4 text-xs leading-relaxed text-amber-800 ring-1 ring-amber-200/60">
                            ...
                            <p><strong>Perhatian:</strong> {{ $role->users_count }} user memakai role ini. Mencabut permission di sini langsung mencabut akses mereka juga.</p>
                        </div>
                    @endif

                    {{-- Actions --}}
                    <div class="space-y-2.5">
                        <p class="flex items-center justify-end gap-1.5 text-xs font-bold text-amber-600" x-show="isDirty" x-cloak>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('roleForm', () => ({
            ...
            originalName: @js(old('name', $role->name)),
            saving: false,
            search: '',
            selected: @js(old('permissions', $role->permissions->pluck('name')->all())),
            ...
            get isDirty() {
                return this.name !== this.originalName || JSON.stringify([...this.selected].sort()) !== JSON.stringify([...this.originalSelected].sort());
            },
