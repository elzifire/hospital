                        ...
                        <tr class="group bg-white transition-colors hover:bg-sky-50/40">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1.5 opacity-60 transition-opacity group-hover:opacity-100 lg:opacity-0">
                                    ...
                                    {{-- Hapus = soft delete (semua pemegang permission). --}}
                                    ...
                                    @if (auth()->user()?->hasRole('superadmin'))
                                        <button type="button" title="Hapus Permanen PNPP"
                                                data-url="{{ route('admin.pnpp.force-destroy', $p) }}"
                                                data-nama="{{ $p->nama }}"
                                                data-kunjungan="{{ $p->kunjungans_count }}"
                                                onclick="hapusPnppPermanen(this)"
                                                class="rounded-lg p-2 text-rose-400 transition-all hover:bg-rose-100 hover:text-rose-700 focus:opacity-100">
                                            <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                                        </button>
                                    @endif
                                </div>
    ...
    function hapusPnpp(el) {
        ...
        confirmSubmit(el.dataset.url, {
            ...
            html: `<div class="text-left text-sm mt-2">
                       <p class="text-slate-600">Apakah Anda yakin ingin menghapus data <strong class="text-slate-900">${nama}</strong> dari daftar PNPP?</p>
                       <p class="mt-3 rounded-lg bg-slate-50 p-2.5 text-xs text-slate-500 ring-1 ring-slate-200">Soft delete — riwayat kunjungan dan penjadwalan tetap tersimpan, dan nama pasien tetap tampil di riwayat.</p>
                   </div>`,
    ...
    function hapusPnppPermanen(el) {
        ...
        const warning = kunjungan > 0
            ? `<div class="flex items-start gap-2.5 rounded-xl bg-amber-50 p-3.5 mt-3 ring-1 ring-amber-200 text-left">
                   ...
                   <p class="text-sm font-medium text-amber-800"><strong>${kunjungan} riwayat kunjungan</strong> (dan penjadwalan terkait) akan ikut terhapus permanen!</p>
               </div>`
            : '';

        confirmSubmit(el.dataset.url, {
            title: 'Hapus Permanen Data PNPP?',
            html: `<div class="text-left text-sm mt-2">
                       <p class="text-slate-600">Apakah Anda yakin ingin menghapus permanen data <strong class="text-slate-900">${nama}</strong>?</p>
                       ${warning}
                       <p class="mt-3 rounded-lg bg-rose-50 p-2.5 text-xs text-rose-600 ring-1 ring-rose-200">Tindakan ini permanen dan tidak dapat dibatalkan.</p>
                   </div>`,
