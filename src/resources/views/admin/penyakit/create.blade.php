...
@section('title', 'Tambah ' . $jenis['label'])
@section('page-title', 'Tambah ' . $jenis['label'])
...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        ...
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Tambah {{ $jenis['label'] }}</h2>
        <p class="mt-0.5 text-sm text-slate-500">Daftarkan jenis {{ strtolower($jenis['label']) }} baru.</p>
    </div>

    <form action="{{ route($jenis['route'] . '.store') }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
