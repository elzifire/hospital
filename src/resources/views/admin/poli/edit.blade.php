...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        ...
        <p class="mt-0.5 text-sm text-slate-500">Perbarui informasi poliklinik.</p>
    </div>
            ...
            <div class="space-y-5 p-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <input type="time" name="jam_buka" id="jam_buka" value="{{ old('jam_buka', $poli->jam_buka?->format('H:i')) }}"
                        ...
                        <div>
                            <input type="time" name="jam_tutup" id="jam_tutup" value="{{ old('jam_tutup', $poli->jam_tutup?->format('H:i')) }}"
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach (\App\Models\Poli::DAFTAR_HARI as $i => $nama)
                            ...
                            <label class="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-sky-300 hover:bg-sky-50/50 has-[:checked]:border-sky-500 has-[:checked]:bg-sky-50 has-[:checked]:text-sky-700">
                                <input type="checkbox" name="{{ $kolom }}" value="1" @checked((bool) old($kolom, $poli->{$kolom})) class="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500">
                                {{ $nama }}
                            </label>
                        @endforeach
                    </div>
