...
<div class="space-y-6">
    ...
    @if ($tab === 'balasan')
        ...
        <form method="GET" action="{{ route('admin.respon.index') }}"
              class="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-xs">
            <input type="hidden" name="tab" value="balasan">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.respon.index', ['tab' => 'balasan']) }}" class="rounded-lg bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-300">Reset</a>
            </div>
        </form>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4">
                <h3 class="text-sm font-bold text-slate-900">Percakapan</h3>
            </div>
                    ...
                    @foreach ($konversasi as $c)
                        ...
                        <li>
                            <a href="{{ route('admin.respon.show', $c->no_hp) }}"
                               class="flex items-center gap-3.5 px-5 py-3.5 transition {{ $adaBelum ? 'bg-sky-50/50 hover:bg-sky-50' : 'hover:bg-slate-50' }}">
                                {{-- Avatar --}}
                                <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full
                                    {{ $adaBelum ? 'bg-sky-100 text-sky-700 font-bold' : 'bg-slate-200 text-slate-500 font-semibold' }}">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if (! $c->pnpp)
                                            <span class="inline-flex flex-shrink-0 rounded-full bg-rose-50 px-1.5 py-0.5 text-[10px] font-semibold text-rose-500">tak terdaftar</span>
                                        @endif
                                    </div>
                                ...
                                <div class="flex flex-col items-end gap-1">
                                    ...
                                    @if ($adaBelum)
                                        <span class="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1.5 text-[10px] font-bold text-white">
                                            {{ $c->belum_dibaca > 99 ? '99+' : $c->belum_dibaca }}
                                        </span>
                                    @endif
                                </div>
                            </a>
