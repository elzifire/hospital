    ...
    <div class="space-y-4"
        ...
        <form method="GET" action="{{ route('admin.respon.pesan-manual') }}"
            ...
            <div class="flex gap-2">
                <button type="submit" class="h-10 rounded-lg bg-sky-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700">Saring</button>
                @if (filled($filters['q']) || filled($filters['satker']))
                    <a href="{{ route('admin.respon.pesan-manual') }}" class="h-10 rounded-lg bg-slate-200 px-4 text-sm font-semibold text-slate-600 transition hover:bg-slate-300">Reset</a>
                @endif
            </div>
        </form>
