...
@section('content')
<div x-data="{ saving: false, selectedWarna: '{{ old('warna', $kategori->warna) }}' }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        <nav class="mb-2 flex items-center gap-1.5 text-xs font-medium text-slate-400" aria-label="Breadcrumb">
            ...
            <span class="font-semibold text-slate-600">{{ $kategori->nama }}</span>
        </nav>
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Edit Kategori Template</h2>
        <p class="mt-0.5 text-sm text-slate-500">Perbarui informasi kategori template.</p>
    </div>

    <form action="{{ route('admin.setting.kategori.update', $kategori) }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
        @method('PUT')
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-100 p-6">
                <h3 class="text-base font-bold text-slate-900">Informasi Kategori</h3>
                <p class="mt-0.5 text-sm text-slate-500">Nama wajib diisi, slug ikut menyesuaikan saat nama diubah.</p>
            </div>
            ...
            <div class="space-y-5 p-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $kategori->nama) }}" required placeholder="cth. Outreach"
                ...
                <div>
                    @include('admin.setting.kategori._warna_picker', ['selectedWarna' => old('warna', $kategori->warna)])
                </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Jelaskan tujuan atau konteks kategori ini..."
                              class="block w-full rounded-xl border-0 py-2.5 px-3.5 text-sm text-slate-900 shadow-xs ring-1 ring-inset ring-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-sky-500 transition @error('deskripsi') ring-rose-300 focus:ring-rose-500 @enderror">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                ...
                <label class="flex cursor-pointer items-center gap-2.5 text-sm font-semibold text-slate-700">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $kategori->is_active) ? 'checked' : '' }}
