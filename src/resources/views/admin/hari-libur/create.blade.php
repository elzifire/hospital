...
<div x-data="{ saving: false }" class="mx-auto max-w-2xl space-y-6">
    ...
    <div>
        ...
        <h2 class="text-xl font-bold tracking-tight text-slate-900">Tambah Hari Libur</h2>
        <p class="mt-0.5 text-sm text-slate-500">Buat hari libur manual. Tanggal kunjungan yang jatuh pada hari ini ditolak pada pendaftaran PNPP.</p>
    </div>

    <form action="{{ route('admin.hari-libur.store') }}" method="POST" @submit="saving = true" x-cloak>
        @csrf
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div class="space-y-5 p-6">
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', request('tanggal')) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required placeholder="cth. Cuti Bersama"
                ...
                <div>
                    ...
                    <select name="poli_id" id="poli_id"
                        ...
                        @foreach ($polis as $poli)
                            <option value="{{ $poli->id }}" @selected((string) old('poli_id') === (string) $poli->id)>{{ $poli->nama }}</option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center justify-end gap-3 rounded-b-2xl border-t border-slate-100 bg-slate-50/70 p-4">
                ...
                <button type="submit" :disabled="saving"
                    ...
                    <span x-text="saving ? 'Menyimpan...' : 'Simpan Hari Libur'"></span>
                </button>
            </div>
        </div>
    </form>
